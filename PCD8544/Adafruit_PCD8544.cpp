...
Adafruit_PCD8544::Adafruit_PCD8544(Adafruit_PCD8544_Ports *ports_v) : Adafruit_GFX(LCDWIDTH, LCDHEIGHT) {
    ports = ports_v;
}
...
void Adafruit_PCD8544::begin(uint8_t contrast, uint8_t bias) {
  ...
  ports->_DIN->setAsOutput();
  ports->_SCKL->setAsOutput();
  ports->_DC->setAsOutput();
  ports->_RST->setAsOutput();
  ports->_CS->setAsOutput();
...
void Adafruit_PCD8544::wake() {
    // get into the EXTENDED mode!
    command(PCD8544_FUNCTIONSET | PCD8544_EXTENDEDINSTRUCTION );

    // LCD bias select (4 is optimal?)
    command(PCD8544_SETBIAS | 0x04);

    command( PCD8544_SETVOP | 55); // Experimentally determined
